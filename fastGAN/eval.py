...
def resize(img):
    return F.interpolate(img, size=224)
...
def batch_save(images, folder_name):
    ...
    for i, image in enumerate(images):
        vutils.save_image(image.add(1).mul(0.5), folder_name+'/%d.png'%i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_sample', type=int, default=2509)
    ...
    for epoch in [10000*i for i in range(args.start_iter, args.end_iter+1)]:
        ckpt = f"Z:/1st/colab_backup/FastGAN/1-fold/20000.pth"
        ...
        dist = f'Z:/1st/colab_backup/FastGAN/1-fold/B'
        ...
        with torch.no_grad():
            for i in tqdm(range(args.n_sample//args.batch)):
                ...
                g_imgs = F.interpolate(g_imgs, 224)

...
def get_all_data_loaders(conf):
    ...
    if 'data_root' in conf:
        train_loader = get_data_loader_folder(os.path.join(conf['data_root'], 'B'), os.path.join(conf['data_root'], 'B'),
                                                batch_size, True, new_size_a, height, width, num_workers, True, datakind)
        test_loader = get_data_loader_folder(os.path.join(conf['data_root'], 'A'), os.path.join(conf['data_root'], 'A'),
                                                batch_size, True, new_size_a, height, width, num_workers, True, datakind)
        # train_loader_b = get_data_loader_folder(os.path.join(conf['data_root'], 'A_live'), os.path.join(conf['data_root'], 'A_live'),
        #                                         batch_size, True, new_size_a, height, width, num_workers, True, datakind)
        # test_loader_b = get_data_loader_folder(os.path.join(conf['data_root'], 'B_live'), batch_size, False,
        #                                      new_size_b, new_size_b, new_size_b, num_workers, True, datakind)
    else:
        train_loader = get_data_loader_folder(os.path.join(conf['data_folder_train_b'], 'A'), os.path.join(conf['data_folder_train_b'], 'A'),
                                                batch_size, True, new_size_a, height, width, num_workers, True, datakind)
        test_loader = get_data_loader_folder(os.path.join(conf['data_folder_train_b'], 'B'), os.path.join(conf['data_folder_train_b'], 'B'),
                                                batch_size, True, new_size_a, height, width, num_workers, True, datakind)
        # train_loader_b = get_data_loader_list(conf['data_folder_train_b'], conf['A_live'], batch_size, True,
        #                                         new_size_b, height, width, num_workers, True, datakind)
        # test_loader_b = get_data_loader_list(conf['data_folder_test_b'], conf['B_live'], batch_size, False,
        #                                         new_size_b, new_size_b, new_size_b, num_workers, True, datakind)
...
def get_data_loader_folder(input_folder_A, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=0, crop=True, datakind=''):
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.5, 0.5, 0.5),
                                           (0.5, 0.5, 0.5))]
    transform_list = [transforms.RandomCrop((height, width))] + transform_list if crop else transform_list
    ...
    transform_list = [transforms.RandomHorizontalFlip()] + transform_list if train else transform_list
    transform = transforms.Compose(transform_list)
    dataset = ImageFolder(input_folder_A, input_folder_A, transform=transform)
    loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=train, drop_last=True, num_workers=num_workers)
    return loader

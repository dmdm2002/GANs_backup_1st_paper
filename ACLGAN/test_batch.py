...
parser.add_argument('--input_folder', type=str, default="Z:/2nd_paper/dataset/ND/original/B", help="input image path")
parser.add_argument('--output_folder', type=str, default="Z:/[1]/backup/ckp/GAN/ND/ACL-GAN/2-fold/test", help="output image path")
parser.add_argument('--checkpoint', type=str,  default="Z:/[1]/backup/ckp/GAN/ND/ACL-GAN/2-fold/outputs/IRIS_DATA/checkpoints/gen_00100000.pt", help="checkpoint of autoencoders")
...
image_names = ImageFolder(opts.input_folder, opts.input_folder, transform=None, return_paths=True)
data_loader = get_data_loader_folder(opts.input_folder, 1, False, new_size=config['new_size'], num_workers=1,crop=False)
...
if opts.trainer == 'aclgan':#    # Start testing
    ...
    for i, ((images_a, images_b), names) in enumerate(zip(data_loader, image_names)):
        if i>=3000:
            break
        if opts.compute_CIS:
            cur_preds = []
        print(names[1])
        images = Variable(images_a.cuda(), volatile=True)

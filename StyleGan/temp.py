...
latent_size = 512
BATCH_SIZE = 6
directory = "E:/Iris_dataset/nd_labeling_iris_data/StyleGAN/2-fold/B"
...
class StyleGAN(object):
    ...
    def evaluate(self, num = 0):
        ...
        for i in range(0, 4554):
            ...
            c1 = np.clip(generated_images[i], 0.0, 1.0)
            x = Image.fromarray(np.uint8(c1*255))

            x.save("E:/backup/StyleGAN/ND/2-fold/test/i_s/i"+str(num)+"_"+str(i)+".png")
        ...
        for i in range(0, 4554):
            ...
            x.save("E:/backup/StyleGAN/ND/2-fold/test/ema/i"+str(num)+"_"+str(i)+"-ema.png")
        ...
        for i in range(0, 4554):
            ...
            x.save("E:/backup/StyleGAN/ND/2-fold/test/mr/i"+str(num)+"_"+str(i)+"-mr.png")
    ...
    def generateTruncated(self, style, noi = np.zeros([44]), trunc = 0.5, outImage = False, num = 0):
        ...
        if outImage:
            ...
            x.save("E:/backup/StyleGAN/ND/2-fold/Results/t"+str(num)+".png")

        return generated_images

    def saveModel(self, model, name, num):
        json = model.to_json()
        with open("E:/backup/StyleGAN/ND/2-fold/Models/"+name+".json", "w") as json_file:
            json_file.write(json)

        model.save_weights("E:/backup/StyleGAN/ND/2-fold/Models/"+name+"_"+str(num)+".h5")

    def loadModel(self, name, num):

        file = open("E:/backup/StyleGAN/ND/2-fold/Models/"+name+".json", 'r')
        json = file.read()
        file.close()

        mod = model_from_json(json, custom_objects = {'Conv2DMod': Conv2DMod})
        mod.load_weights("E:/backup/StyleGAN/ND/2-fold/Models/"+name+"_"+str(num)+".h5")

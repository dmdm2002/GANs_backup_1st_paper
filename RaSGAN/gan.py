...
import cv2
from RaSGAN.utils import save_model
import torchvision.transforms as transform
import numpy as np
import os

from RaSGAN.fid_score import calculate_fid
...
class GAN:
    ...
    def optimize_discriminator_net(self, real_data, fake_data, real_labels, fake_labels):
        ...
        real_labels=real_labels.to(self.device,dtype=torch.float32)
    ...
    def train(self, data):
        ...
        for epoch in range(self.params["num_epochs"]):
            output = f'E:/backup/ckp/GAN/rasgan/2/test/original_size/second/{epoch}'
            if epoch > 250:
                os.mkdir(output)
            ...
            for i, train_data in enumerate(data):
                ...
                if epoch > 250:
                    with torch.no_grad():
                        fake_images = self.generator_net(noise).detach().cpu()
                        for j in range(len(fake_images)):
                            path = f'{output}/iter_{i}_batch_{j}.png'
                            vutils.save_image(fake_images, path)
    ...
    def testing(self, data):
        ...
        # todo: load models
        # For each epoch
        # noise = torch.randn(1, self.params["vector_size"], 1, 1, device=self.device)
        for epoch in range(1):
            ...
            # for i, train_data in enumerate(data):
            noise = torch.randn(len(data), self.params["vector_size"], 1, 1, device=self.device)
            data_length = len(data)
            # print(f"Iteration [{i}] / [{data_length}]")
            with torch.no_grad():
                fake_images = self.generator_net(noise).detach().cpu()
                for j in range(len(fake_images)):
                    path = f'{output}/{j}.png'
                    vutils.save_image(fake_images[j], path)

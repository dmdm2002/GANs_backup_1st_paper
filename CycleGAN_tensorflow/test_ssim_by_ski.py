...
for i in range(92, 93):
    ...
    for n, (input_image, target) in zipeed_dataset.enumerate():
        ...
        for i in range(args.batch_size):
            ssim2=ssim(input_image[i].numpy(),target[i].numpy(),win_size=11,gaussian_weights=True,multichannel=True)
            #ssim2 = tim.ssim(input_image, target, max_val=1.0, filter_size=11,
            #                 filter_sigma=1.5, k1=0.01, k2=0.03)

            name = py.name(A_img_paths[idx])
            f = open('generate_2fold(B)_2/all_92_img_ssim_by_ski.csv', 'a', newline='')
            wr = csv.writer(f)
            wr.writerow([name, ssim2])
            f.close()
